...
func add_variable(new_name='', new_type=0, new_value = ''):
	...
	var _res = var_name.connect("text_changed", Callable(self, "_on_modified"))
	...
	_res = type.connect("item_selected", Callable(self, "_on_modified"))
	...
	_res = var_val.connect("text_changed", Callable(self, "_on_modified"))
